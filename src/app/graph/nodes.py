...
def fallback_node(state: AgentState) -> dict[str, Any]:
    """
    Graceful fallback when the rewrite loop is exhausted.

    After max_rewrite_retries, the agent stops retrying and returns
    an honest 'I don't have documentation for this' response.
    """
    question = state["question"]
    steps = list(state.get("steps", []))
